function onmessage(event) {
	db = loadAndClearDatabase();
	fileStream = Ti.Filesystem.getFileStream(event.message.fileName); 
	if (fileStream.open(Ti.Filesystem.MODE_READ)) {
		processRows(db, fileStream);
    }
}

function processRows(db, fileStream) {

	var line = '';
	var i = 0;
	var batch = 1;
	while (batch == 1 || line != null) {
		db.execute("BEGIN TRANSACTION"); 
		while ( ( line = fileStream.readLine() ) != null && i < (batch * 100)) {
			// parseCSV function returns an array of arrays, but we're only parsing one row at a time
			var array = parseCSV(String(line))[0];
			db.execute("INSERT INTO Rows VALUES(?,?,?,?,?,?)", 
				array[0].replace(/"/g,''),  
				array[1].replace(/"/g,''), 
				array[2].replace(/"/g,''), 
				array[3].replace(/"/g,''), 
				String(parseFloat(array[4].replace(/"/g,'')).toFixed(2)),
				moment(array[5].replace(/"/g,''), 'DD/MM/YYYY').format("YYYY-MM-DD")
			)
			i++;
		}
		db.execute("COMMIT TRANSACTION");
		this.postMessage({progress: i });
		batch++;
	}
    this.postMessage({complete: true });

}

function loadAndClearDatabase() {
	var db = Ti.Database.open('gift_aid_db');
	db.execute("DROP TABLE IF EXISTS Rows");
	db.execute("CREATE TABLE IF NOT EXISTS Rows(firstName TEXT, lastName TEXT, houseNumber TEXT, postCode TEXT, amount TEXT, date TEXT)");
